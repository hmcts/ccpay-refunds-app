...
resource "azurerm_key_vault_secret" "fee_pay_team_refund_subscription_key" {
  ...
  depends_on = [azurerm_api_management_subscription.fee_pay_team_refund_subscription]
}

# Supplier subscription - Liberata
resource "azurerm_api_management_subscription" "liberata_supplier_refund_subscription" {
  api_management_name = local.cft_api_mgmt_name
  resource_group_name = local.cft_api_mgmt_rg
  product_id          = module.cft_api_mgmt_product.id
...
resource "azurerm_key_vault_secret" "liberata_supplier_refund_subscription_key" {
  name         = "liberata-cft-apim-refund-subscription-key"
  value        = azurerm_api_management_subscription.liberata_supplier_refund_subscription.primary_key
  key_vault_id = data.azurerm_key_vault.refunds_key_vault.id

  depends_on = [azurerm_api_management_subscription.liberata_supplier_refund_subscription]
}
